...
impl Thunk {
    ...
    pub async fn force(self, co: GenCo, span: LightSpan) -> Result<Value, ErrorKind> {
        ...
        match inner {
            ...
            ThunkRepr::Suspended {
                ...
                let value = generators::request_enter_lambda(&co, lambda, upvalues, light_span).await;
    ...
    pub fn upvalues_mut(&self) -> RefMut<'_, Upvalues> {
        RefMut::map(self.0.borrow_mut(), |thunk| match thunk {
            ...
            ThunkRepr::Evaluated(Value::Closure(c)) => Rc::get_mut(&mut Rc::get_mut(c).unwrap().upvalues)
